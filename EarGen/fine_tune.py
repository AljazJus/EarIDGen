...
def main_train(config):
    ...
    if hasattr(config, 'RESUME_FROM_CHECKPOINT') and config.RESUME_FROM_CHECKPOINT:
        ...
        # Try to find the UNet weights with different possible filenames
        checkpoint_files = [
            os.path.join(config.CHECKPOINT_PATH, "diffusion_pytorch_model.bin"),
            os.path.join(config.CHECKPOINT_PATH, "model.bin"),
            os.path.join(config.CHECKPOINT_PATH, "diffusion_pytorch_model.safetensors"),
            config.CHECKPOINT_PATH  # In case the full path to the file was provided
        ]
        ...
        if not checkpoint_found:
            print(f"WARNING: Could not find weights at {config.CHECKPOINT_PATH}")
            print("Training will start from the base pretrained model.")

    with open(os.path.join(output_dir, "training_continuation_log.txt"), "w") as log_file:
        log_file.write(f"Continuing training from: {config.CHECKPOINT_PATH}\n")
        log_file.write(f"New identity loss weight: {config.IDENTITY_LOSS_WEIGHT}\n")
        log_file.write(f"Learning rate: {config.LEARNING_RATE}\n")
        log_file.write(f"Additional epochs: {config.NUM_TRAIN_EPOCHS}\n")
